...
test.describe("validate Deez Notes app", () => {
  ...
  test.describe("validate initial landing page state", () => {
    test("validate header", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="logo-text"]')).toContainText(
        "DEEZ Notes",
      );
      await expect(page.locator('[data-testid="logo-link"]')).toHaveAttribute(
        "href",
        "/",
      );
    ...
    test("validate main content", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="note-text-input"]'),
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="note-text-input"]'),
      ).toHaveAttribute("placeholder", "Type your notes here...");
    });

    test("validate sidebar", async ({ page }) => {
      // Sidebar should be initially closed
      await expect(
        page.locator('[data-testid="sidebar-group-content"]'),
      ).toHaveCount(0);
      ...
      await expect(
        page.locator('[data-testid="sidebar-unauthenticated-text"]'),
      ).toBeVisible();
    });
  });
  ...
  test.describe.skip("login", () => {
    ...
    test("login form validation", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="auth-form"]')).toBeVisible();
      await expect(page.locator('[data-testid="sign-up-button"]')).toHaveCount(
        0,
      );
