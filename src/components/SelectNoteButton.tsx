...
function SelectNoteButton({ note }: Props) {
  ...
  const noteText = isEmptyNote ? blankNoteText : localNoteText || blankNoteText;
  ...
  return (
    <SidebarMenuButton
      ...
      data-testid={`select-note-button ${note.id}`}
    >
      <Link href={`/?noteId=${note.id}`} className="flex h-fit flex-col">
        {/* Display Date and Time */}
        <p className="text-muted-foreground/70 text-xs" data-testid="note-date">
          ...
          className={`mt-2 px-4 ${
            isEmptyNote
              ...
              : note.id === noteId
                ? "opacity-100"
                : "opacity-10"
          } w-full truncate overflow-hidden text-ellipsis whitespace-nowrap transition-opacity duration-1000 ease-out group-hover/item:opacity-100 group-hover/item:duration-200 group-hover/item:ease-in`}
