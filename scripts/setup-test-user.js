  ...
  try {
    ...
    if (error) {
      if (error.message.includes("already registered")) {
        ...
        console.log(
          "🎯 User is ready for testing (email verification disabled)",
        );
      } else {
        throw error;
      }
      ...
      console.log(
        "🎯 User is ready for testing immediately (email verification disabled)",
      );
    }

    console.log("\n🎯 Next steps:");
    console.log(
      '1. Run: npm run cypress -- --spec "cypress/e2e/auth-flow.cy.ts"',
    );
